...
{
    public class ListPopCommand : AsmCommand
    {
        public ValueStorageList source = null;
        public ValueStorage target = null;

        public ListPopCommand(AsmInterpreter interpreter, string[] lineParts)
            : base(interpreter, lineParts)
        {
            ...
            if (vsl == null)
                throw new InvalidOperationException("Invalid pop source.");
            if (vs == null)
        ...
        {
            target.SetValue(source.Pop().GetValue());
            base.Run();
        }
    }
}
